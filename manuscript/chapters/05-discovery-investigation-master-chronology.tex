...
These counts are taken from the master chronology database, event-source map, and chronology uncertainty register. They describe the admitted project chronology, not every event claimed in prior literature or public summaries.
...
Exclusion from this chronology does not prove that a proposed pre-1948 event is false. It means only that the event was not verified or partially verified to the admission standard used here.
...
\hbadness=10000
\hfuzz=40pt
\input{manuscript/tables/timeline_ready_for_latex}
\endgroup
